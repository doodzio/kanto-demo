...
class CommandsInvoker(MessagingHandler):
    ...
    def on_sendable(self, event):
        data = dict()
        if container_img_ref: 
            with open(container_config_file) as container_config:
        ...
        if command == CLI_OPT_RUN_CMD:
            ...
            value = json.dumps(dict(imageRef=container_img_ref, start=True, config=container_config))
        else:
            action = 'remove'
...
options, reminder = getopt.getopt(sys.argv[2:], 't:d:', ["img=", "spec=", "id="])
opts_dict = dict(options)
tenant_id = os.environ.get("TENANT") or opts_dict['-t']
device_id = os.environ.get("DEVICE_ID") or opts_dict['-d']
command = sys.argv[1]
if command == CLI_OPT_RUN_CMD:
    container_img_ref = opts_dict['--id']
    container_config_file = opts_dict['--spec']
