...
class Kuksa_Client():
    ...
    def setPosition(self, position):
        print(position)
        self.client.setValue('Vehicle.Cabin.Infotainment.Navigation.CurrentLocation.Altitude', str(position['alt']))
        self.client.setValue('Vehicle.Cabin.Infotainment.Navigation.CurrentLocation.Latitude', str(position["lat"]))
        self.client.setValue('Vehicle.Cabin.Infotainment.Navigation.CurrentLocation.Longitude', str(position["lon"]))
        self.client.setValue('Vehicle.Cabin.Infotainment.Navigation.CurrentLocation.Accuracy', str(position["hdop"]))
        self.client.setValue('Vehicle.Cabin.Infotainment.Navigation.CurrentLocation.Speed', str(int(position["speed"])))
...
class GPSD_Client():
    def __init__(self, config, consumer):
        ...
        self.gpsd_port=provider_config.get('port','2948')
        self.interval = provider_config.getint('interval', 1)

        self.gpsd = gps.gps(host = self.gpsd_host, port = self.gpsd_port, mode=(gps.WATCH_ENABLE | gps.WATCH_JSON))
            ...
            try:
                if self.gpsd.waiting():
                    ...
                    if report['class'] == 'TPV':
                        ...
                        self.position['speed'] = getattr(report, 'speed', 0)
